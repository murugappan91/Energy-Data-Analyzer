...
/**
 *
 * Servlet to validate user login for registered users.
 */
public class LoginServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		try {
			...
			String checklogin = "select [UID],[Pass] from [UserReg] where [UID]='"
					+ user + "' and [Pass]='" + pass + "'"; 
			...
			//authentication failed: redirect to loginfailed page
			if (check == null) {
				
				String contextPath = "http://localhost:8084/Project/LoginFailed.html?user="
						+ user;
				response.sendRedirect(response.encodeRedirectURL(contextPath));

			} else {
			//authentication success - redirect user to homepage
